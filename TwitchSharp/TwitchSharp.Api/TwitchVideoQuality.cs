...
namespace TwitchSharp.Api
{
    public class TwitchVideoQuality : IComparable<TwitchVideoQuality>
    {
        ...
        private int? GetFps(string qualityId, string fps)
        {
            ...
            {
                decimal qualityFpsDec;
                int? qualityFps = decimal.TryParse(qualityId.Substring(start, qualityId.Length - start), out qualityFpsDec) ? (int?)Math.Round(qualityFpsDec, 0) : null;
            ...
            decimal fpsDec;
            return decimal.TryParse(fps, out fpsDec) ? (int?)Math.Round(fpsDec, 0) : null;
        }
